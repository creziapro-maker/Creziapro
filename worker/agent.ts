...
export class ChatAgent extends Agent<Env, ChatState> {
  ...
  async onStart(): Promise<void> {
    // The chat handler is now initialized in onRequest to get the request origin
  ...
  async onRequest(request: Request): Promise<Response> {
    try {
      // Initialize ChatHandler here to get access to the request origin for API calls
      if (!this.chatHandler) {
        ...
        this.chatHandler = new ChatHandler(
          ...
          this.state.model,
          controller as DurableObjectStub<AppController>
        );
      }
