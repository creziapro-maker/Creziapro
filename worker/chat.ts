...
import type { Message, ToolCall, Service } from './types';
...
export class ChatHandler {
  ...
  private controller: DurableObjectStub<AppController>;
  constructor(aiGatewayUrl: string, apiKey: string, model: string, controller: DurableObjectStub<AppController>) {
  ...
  async processMessage(
    ...
    const messages = await this.buildConversationMessages(message, conversationHistory);
    ...
    const followUpCompletion = await this.client.chat.completions.create({
      ...
      messages: [
        ...
        { role: 'user', content: userMessage },
        {
          role: 'assistant',
          content: null,
          tool_calls: openAiToolCalls as OpenAI.Chat.Completions.ChatCompletionMessageToolCall[]
        },
   ...
   */
  private async buildConversationMessages(userMessage: string, history: Message[]) {
    let systemPrompt = 'You are a helpful AI assistant for Creziapro.'; // Default fallback
    try {
      const config = await this.controller.getChatbotConfig();
      const { prompt, services } = config;
      const servicesInfo = services
        .map(
          (s: Service) =>
            `- ${s.title}: ${s.description}\n  Pricing: ${s.pricingBands
              .map(p => `${p.label} (₹${p.min} - ₹${p.max})`)
              .join(', ')}`
        )
        .join('\n');
      systemPrompt = `${prompt}\n\nHere is the list of services and pricing information you must use:\n${servicesInfo}`;
    } catch (error) {
      console.error('Failed to fetch dynamic chatbot config from AppController:', error);
    }
    return [
      {
